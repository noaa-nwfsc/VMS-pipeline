...
fredr_set_key('3101834d945b5dddb0e6004862a11183') # The Fred API key Blake created at https://fred.stlouisfed.org/docs/api/api_key.html
fred_gdpdefl <- fredr(
  series_id = "GDPDEF",
  observation_start = as.Date("1985-01-01")
)

# generate the mean annual deflators using the quarterly values, using the reference year defined in main script for afi_year
gdp_defl <- mutate(fred_gdpdefl, YEAR = format(date,"%Y")) %>%
  filter(YEAR <= afi_year) %>%
  group_by(YEAR) %>%
