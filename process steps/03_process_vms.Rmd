...
Read in the bathymetry object

```{r}
bathy.grid <- rast(here::here('data','raw','grid','bathy_raster_lamb.tif'))
```

Get bathymetry at VMS data points

```{r}
vms_sp <- vms_nodup %>% st_as_sf(coords=c("LON","LAT"),crs=4326) %>% st_transform(crs(bathy.grid))
