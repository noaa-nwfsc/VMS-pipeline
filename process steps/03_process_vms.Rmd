...
vms_fn <- list.files(here('data','Confidential','raw','vms'),full.names = T)
...
write_rds(vms_dupes, here::here('data','Confidential','processed','vms',paste0(process_year, "_duplicates_only.rds")))
```
...
```{r}
bathy.grid <- rast(here::here('data','Confidential','raw','grid','composite_bath.tif'))
```
...
```{r save rds}
write_rds(vms_ordered,here::here('data','Confidential','processed','vms',paste0(process_year,'_vms_clean.rds')))
```
