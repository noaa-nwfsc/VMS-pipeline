...
for(y in unique(vessel_length_key_df$year)){
  key_subset <- vessel_length_key_df %>%
    filter(year == y)
  write_rds(key_subset,here::here('Confidential','processed','vessel length keys',paste0(spp_codes,'_vessel_length_key_', y, '.rds')))
}
```
...
for(y in unique(vessel_length_key_df$year)){
  ...
  write_rds(landings_length,here::here('Confidential','processed','fish tickets',paste0(spp_codes,'_',y, 'fishtix_vlengths_withFTID.rds')))
}
```
