...
```{r}
permits <- read_csv(here('Confidential','raw','vessel_registration','2009_2023_vesselreg.csv'))
```
...
landings <- purrr::map(process_year, function(y){
  read_rds(here::here('Confidential','processed','fish_tickets',paste0(y,'fishtix_withFTID.rds')))
}) %>% 
  bind_rows()
...
for(y in unique(vessel_length_key_df$year)){
  ...
  write_rds(landings_length,here::here('Confidential','processed','fish_tickets',paste0(y, 'fishtix_vlengths_withFTID.rds')))
}
```
