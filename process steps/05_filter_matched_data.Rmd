...
keepers <- c('process_year','alltime','spp_codes','gear_codes','pacfin_weight_metric','pacfin_revenue_metric','target_cutoff','lookback_window','year_begin','year_end','lookback_begin', 'interpolate_flag')
...
```{r data}
vms <- read_rds(here::here('Confidential','data','processed','matched','matching',paste0(process_year,'_matched_vmstix_only_withFTID.rds')))
```
...
```{r}
portlist_coords <- read_csv(here::here('spatial_data','port_coords_fromBlake_edited.csv'),col_types='cddd') %>% 
...
write_rds(vms_filtered,here::here('Confidential','data','processed','matched','filtering',paste0(process_year,'_matched_filtered_withFTID_length.rds')))
write_rds(vms_wfilters,here::here('Confidential','data','processed','matched','filtering',paste0(process_year,'_matched_unfiltered.rds')))
```
