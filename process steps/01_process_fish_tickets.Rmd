...
keepers <- c('process_year','alltime','spp_codes','gear_codes','pacfin_weight_metric','pacfin_revenue_metric','target_cutoff','lookback window','year_begin','year_end','lookback_begin')
...
rawdat <- read_rds(here::here('Confidential','data','raw','fish tickets','all_fishtickets_1994_2023.rds'))
rawdat <- rawdat %>%
...
```{r}
rawdat.sub <- rawdat %>% 
  dplyr::select(FISH_TICKET_ID, FTID, PACFIN_PORT_CODE, PACFIN_GROUP_PORT_CODE, VESSEL_NUM, AGENCY_CODE, GEAR_CODE, GEAR_NAME, PACFIN_GROUP_GEAR_CODE, REMOVAL_TYPE_CODE, REMOVAL_TYPE_NAME, LANDING_DATE, LANDING_MONTH, LANDING_YEAR, PACFIN_SPECIES_CODE, LANDED_WEIGHT_LBS, EXVESSEL_REVENUE, AFI_EXVESSEL_REVENUE) %>% 
