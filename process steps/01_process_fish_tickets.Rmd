...
keepers <- c('process_year','first_process_year','alltime','spp_codes','gear_codes','pacfin_weight_metric','pacfin_revenue_metric','target_cutoff','lookback_window','year_begin','year_end','lookback_begin', 'interpolate_flag')
...
```{r}
rawdat.sub <- rawdat %>% 
  dplyr::select(FISH_TICKET_ID, FTID, PACFIN_PORT_CODE, PACFIN_GROUP_PORT_CODE, DEALER_ID, VESSEL_NUM, AGENCY_CODE, GEAR_CODE, GEAR_NAME, PACFIN_GROUP_GEAR_CODE, REMOVAL_TYPE_CODE, REMOVAL_TYPE_NAME, LANDING_DATE, LANDING_MONTH, LANDING_YEAR, PACFIN_SPECIES_CODE, LANDED_WEIGHT_LBS, EXVESSEL_REVENUE) %>% 
...
# 1. select all columns of interest and match by c(Rec_ID,PACFIN_SPECIES_CODE)
#   recID_attributes <- rawdat.sub %>%
#     select(Rec_ID,FTID,PACFIN_SPECIES_CODE,pacfin_port_code,port_group_code,dealerID,gear_code,gear_name,gear_group,drvid,agency_code,date,month,year)
# 2. select all columns and match by c(Rec_ID,PACFIN_SPECIES_CODE)
#   recID_attributes2 <- rawdat.sub %>% 
#     distinct(Rec_ID,FTID,PACFIN_SPECIES_CODE,pacfin_port_code,port_group_code,dealerID,gear_code,gear_name,gear_group,drvid,agency_code,date,month,year)
...
# Test the join and count
  ...
  # rawdat.w.targets.test1 %>% count(Rec_ID,PACFIN_SPECIES_CODE) %>% pull(n) %>% range()
# for 2017 data- range is 1:40.

  # rawdat.w.targets.test2 <- rawdat.w.targets %>% 
  #   left_join(recID_attributes2,by=c("Rec_ID","PACFIN_SPECIES_CODE"))
  # rawdat.w.targets.test2 %>% count(Rec_ID,PACFIN_SPECIES_CODE) %>% pull(n) %>% range()
# for 2017 data- range is 1:2.

  # rawdat.w.targets.test3 <- rawdat.w.targets %>% 
  #   left_join(recID_attributes3,by=c("Rec_ID","PACFIN_SPECIES_CODE"))
  # rawdat.w.targets.test3 %>% count(Rec_ID,PACFIN_SPECIES_CODE) %>% pull(n) %>% range()
...
# CAUTION: this subsection will subset all fish tix just to groundfish bottom trawl
...
RecID_spp_cols <- dat_targets_other %>% 
  select(Rec_ID,PACFIN_SPECIES_CODE,spp_lbs,spp_lbs_no_revenue,spp_revenue) %>% 
