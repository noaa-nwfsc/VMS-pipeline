...
dcrb_vms_tix_analysis <- vms_all_grd_match %>%
  ...
  dplyr::select(
    ...
    TARGET_lbs, TARGET_rev, DCRB_lbs, DCRB_revenue, other_lbs, other_rev, species_code_all
    ) # DEPTH_CATM, 
...
dcrb_vms_tix_analysis_TripInfo <- left_join(VMSrecords_per_trip, dcrb_vms_tix_analysis, by="Rec_ID") %>%
  mutate(
    ...
    other_lbs_per_VMSlocation = other_lbs/trip_VMSrecords,
    other_rev_per_VMSlocation = other_rev/trip_VMSrecords,
...
con_df_daily_years_2km <- dcrb_vms_tix_analysis_TripInfo %>%
  ...
  summarise(
    ...
    other_lbs = sum(other_lbs_per_VMSlocation),
    other_rev = sum(other_rev_per_VMSlocation),
...
summarise(DCRB_lbs = sum(DCRB_lbs, na.rm = TRUE),
    ...
    #other_lbs = sum(other_lbs, na.rm = TRUE),
    #other_rev = sum(other_rev, na.rm = TRUE),
    Num_DCRB_VMS_pings = n())
