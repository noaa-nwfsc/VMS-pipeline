...
# create variable for years of data to load
load_years <- c(2014, 2015, 2016)
# make sure to change crab_years in the next code chunk too
...
crab_years <- c("2014_2015", "2015_2016") # crab years to include
...
# TODO should this be by crab_year rather than by year?
...
fishing_activity_monthly_table <- dcrb_5km_output_df %>%
  ...
  arrange(year, month_numeric)
# write result
write.csv(file=here("Confidential", "tables", "table_fishing_activity_monthly.csv"), x=fishing_activity_monthly_table)
...
fishing_activity_yearly_table <- dcrb_5km_output_df %>%
  ...
  arrange(year)
# write result
write.csv(file=here("Confidential", "tables", "table_fishing_activity_yearly.csv"), x=fishing_activity_yearly_table)
```

```{r maps}
# map fishing effort, apply to each month of data
#   data_sf: sf object
...
fishing_effort_map <- function (data_sf, data_col, bbox, log_transform, fill_range, title, data_col_name, png_suffix='', background_sf) {
...
for (ym in month_df$year_month) {
  ...
  vms_map <- fishing_effort_map(data_sf = ym_data_sf,
  ...
  vessels_map <- fishing_effort_map(data_sf = ym_data_sf,
