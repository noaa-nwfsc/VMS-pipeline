...
```{r confidentiality-filter}
# create a non-confidential version of gridded fishing summary
dcrb_5km_summary_confidential_df <- dcrb_5km_summary_df %>%
  filter(n_unique_vessels >= 3)

# summarize how much data is dropped, where a record is a VMS pings
# numerator: # of records dropped due to confidentiality filter
n_drop_confidential <- sum(dcrb_5km_summary_df$n_vms_records) - sum(dcrb_5km_summary_confidential_df$n_vms_records)
# denominator: # of records, without any confidentiality filter
n_no_drop_confidential <- sum(dcrb_5km_summary_df$n_vms_records)

# calculate the number of records, and convert to a percent as a string
percent_drop_confidential <- substr(as.character(n_drop_confidential / n_no_drop_confidential * 100), 1, 5)
```

The non-confidential data has `r n_drop_confidential` (`r percent_drop_confidential`%) fewer records (VMS pings), where grid cells had <3 unique vessels in a given month.
...
summary_table <- rbind(
  summarise_at(dcrb_5km_summary_confidential_df, colnames_vector, get(function_vector[1])),
  summarise_at(dcrb_5km_summary_confidential_df, colnames_vector, get(function_vector[2])),
  summarise_at(dcrb_5km_summary_confidential_df, colnames_vector, get(function_vector[3])),
  summarise_at(dcrb_5km_summary_confidential_df, colnames_vector, get(function_vector[4])),
  summarise_at(dcrb_5km_summary_confidential_df, colnames_vector, get(function_vector[5])),
  summarise_at(dcrb_5km_summary_confidential_df, colnames_vector, get(function_vector[6]))
)
...
```{r maps}
# create a log-10 transformed map
#   data_sf: sf object
...
create_log10_map <- function (data_sf, data_col, bbox, data_col_name, png_suffix='', background_sf) {
  # create map
  map_output <- ggplot() +
    # plot background
    ...
    # label plot
    labs(fill=paste0("log10(", data_col_name, ") per 5x5 cell")) +
    # aesthetic changes
...
dcrb_5km_summary_confidential_sf <- dcrb_5km_summary_confidential_df %>%
...
# get unique and ordered year (numeric), year_month (character), and month (factor)
month_df <- dcrb_5km_summary_confidential_sf %>% 
  st_drop_geometry() %>% 
...
for (ym in month_df$year_month) {
  ...
  ym_data_sf <- dcrb_5km_summary_confidential_sf %>% filter(year_month == ym)
  # make and save map
