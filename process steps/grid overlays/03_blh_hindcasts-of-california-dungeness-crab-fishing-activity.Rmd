...
print(summary(dcrb_5km_summary_sf$rescaled_n_vms_records))
print(summary(dcrb_5km_summary_sf$normalized_n_vms_records))

# join geometry from 5km grid to gridded fishing summary
dcrb_5km_summary_sf <- dcrb_5km_summary %>%
  left_join(grid_5km, by=join_by(GRID5KM_ID)) %>%
  st_as_sf()
```

I still need to add a filter to remove confidential grid cells.
...
# set bounding box 
bbox <- st_bbox(dcrb_5km_summary_sf)
...
map_density <- ggplot() +
  ...
  # plot fishing effort
  geom_sf(data=dcrb_5km_summary_sf, aes(fill=log10(n_vms_records))) +
  # label plot
...
map_density <- ggplot() +
  ...
  geom_sf(data=west_coast_states, fill='gray80') +
  # plot fishing effort
  geom_sf(data=dcrb_5km_summary_sf, aes(fill=log10(rescaled_n_vms_records))) +
  # label plot
...
map_density <- ggplot() +
  ...
  geom_sf(data=west_coast_states, fill='gray80') +
  # plot fishing effort
  geom_sf(data=dcrb_5km_summary_sf, aes(fill=log10(normalized_n_vms_records))) +
  # label plot
