...
Before post-processing filtering, VMS data includes `n_records` records (VMS pings), `n_trips` trips (fish tickets), across `n_years` years.
`n_interpolated_records` records (`round(n_interpolated_records / n_records * 100, 0)`%) are interpolated, and `n_interpolated_trips` trips (`round(n_interpolated_trips / n_trips * 100, 0)`%) have at least one interpolated record.
...
After post-processing filtering, VMS data includes `n_records` records (VMS pings), `n_trips` trips (fish tickets), across `n_years` years.
