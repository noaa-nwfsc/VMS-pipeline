...
```{r}
# set boolean flag for whether to use interpolated or non-interpolated VMS pings
...
rm(list = setdiff(ls(), 'use_interpolated'))

# load 2014-2023 VMS data
if (use_interpolated) {
  # load interpolated data
  vms_all <- read_rds(here('Confidential', 'processed', 'matched', 'interpolation', '2014_interpolated.rds')) %>%
...
# count records, trips and years
n_records <- nrow(vms_all)
n_trips <- n_distinct(vms_all$Rec_ID)
n_years <- n_distinct(lubridate::year(vms_all$westcoastdate_notime))

# count interpolated records and trips
if (use_interpolated) {
  n_interpolated_records <- vms_all %>% filter(is_interpolated == 1) %>% select(VMS_RECNO) %>% n_distinct()
  n_interpolated_trips <- vms_all %>% filter(is_interpolated == 1) %>% select(Rec_ID) %>% n_distinct()
} else {
  n_interpolated_records <- 0
  n_interpolated_trips <- 0
}
```

Before post-processing filtering, VMS data includes `r n_records` records (VMS pings), `r n_trips` trips (fish tickets), across `r n_years` years.
`r n_interpolated_records` records (`r round(n_interpolated_records / n_records * 100, 0)`%) are interpolated, and `r n_interpolated_trips` trips (`r round(n_interpolated_trips / n_trips * 100, 0)`%) have at least one interpolated record.

### Match VMS pings and fish tickets to grid cells

```{r}
# remove counts from above
rm(n_records, n_trips, n_years, n_interpolated_records, n_interpolated_trips)
...
# count records, trips and years
n_records <- nrow(dcrb_vms_tix_analysis)
n_trips <- n_distinct(dcrb_vms_tix_analysis$Rec_ID)
n_years <- n_distinct(dcrb_vms_tix_analysis$year)
```

After post-processing filtering, VMS data includes `r n_records` records (VMS pings), `r n_trips` trips (fish tickets), across `r n_years` years.
...
* For example, in a 12 hour trip with 48 pings, `avg_trip_ping_rate` = 48/12 = 4 pings/hour, and `VMSrecords_adjusted_trip` = 1/4 = 0.25 adjusted pings.
 ...
 Then, normalize adjusted VMS pings by year-month and grid cell, and attribute landings and revenue across grid cells.
