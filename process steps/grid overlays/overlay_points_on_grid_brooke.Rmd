...
# look at average pings per ticket over years in table
avg_pings_per_ticket_over_time <- dcrb_vms_tix_analysis %>%
  group_by(year, Rec_ID) %>%
  summarise(trip_VMSrecords = n()) %>%
  ungroup() %>%
  group_by(year) %>%
  summarise(avg_pings_per_ticket = mean(trip_VMSrecords))

# look at average pings per ticket over years in scatterplot
avg_pings_per_ticket_over_time %>% 
  ggplot(aes(x = year, y = avg_pings_per_ticket)) +
  geom_point() +
  theme(axis.text.x = element_text(angle = 270, vjust = 0.5, hjust=1))

# so it was 2021, now find out which month...
...
# look at average pings per ticket over time in scatterplot
avg_pings_per_ticket_over_time %>% 
  ggplot(aes(x = month, y = avg_pings_per_ticket)) +
  geom_point() +
  theme(axis.text.x = element_text(angle = 270, vjust = 0.5, hjust=1))

# so it was January, now look for which day...
...
# look at average pings per ticket over time in scatterplot
avg_pings_per_ticket_over_time %>% 
  ggplot(aes(x = westcoastdate_notime, y = avg_pings_per_ticket)) +
  geom_point() +
  theme(axis.text.x = element_text(angle = 270, vjust = 0.5, hjust=1))
...
# possibly Jan. 7? but want to see side by side with # tickets
