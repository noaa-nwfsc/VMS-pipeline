...
Create outputs for Heather.

* *Baseline 2014-2018* includes the 2014-15 to 2018-19 fishing seasons.
* *Baseline 2017-2018* includes the 2017-18 and 2018-19 fishing seasons.
* *Implementation 2019-2023* includes the 2019-20 to the end of the 2023 calendar year.

For baseline periods, each record represents one grid cell for one month, across all years. Crab data is averaged across years for a given month. Note that the # of unique vessels is the minimum for that grid cell and month in any year, let me know if I should update this.
...
```
```{r investigate 2023 lack of data}
# load 2023 data
...
# check # records vs. year, crab_year, and month before transformation
vms_2023 %>% 
  mutate(year = lubridate::year(westcoastdate_notime),
         month_as_numeric = month(westcoastdate_notime),
         crab_year = ifelse(
           month_as_numeric >= 11, paste0(year, "_", 1+year), paste0(year-1, "_", year)
         )
      ) %>%
  group_by(year, crab_year, month_as_numeric) %>%
...
# filter same as earlier in script
