...
# TODO update below this line to summarise and visualize both interpolated and non-interpolated data
```
...
map_adjusted_pings <- ggplot() +
  ...
  geom_sf(data = dcrb_year_month_5km_df_with_grid, aes(fill = log10(DCRB_VMS_pings_adjusted)), color = NA) +
...
map_adjusted_normalized_pings <- ggplot() +
  geom_sf(data = coaststates, fill = 'gray80') +
  geom_sf(data = dcrb_year_month_5km_df_with_grid, aes(fill = log10(normalized_adjusted_DCRB_VMS_pings)), color = NA) +
  facet_grid(. ~ crab_year) +
  scale_fill_viridis() +
  xlim(bbox[1], bbox[3]) + ylim(bbox[2], bbox[4]) +
