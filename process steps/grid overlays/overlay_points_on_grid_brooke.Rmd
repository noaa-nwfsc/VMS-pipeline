...
# TODO write a function to visualize maps, with baseline or yearly data, with adjusted or not pings

# visualize all years (non-adjusted ping rate)
...
# visualize all years (adjusted ping rate with monthly adjustment)
# join summary back to geometry by GRID5KM_ID
dcrb_year_month_5km_df_with_grid <- dcrb_year_month_5km_df %>% 
  left_join(grd, by = join_by(GRID5KM_ID)) %>%
  st_as_sf()
...
bbox <- st_bbox(dcrb_year_month_5km_df_with_grid)
# plot log of avg. pings on a map
map_log_pings_dcrb_year_month_5km_df_adjusted <- ggplot() +
  # background map
  geom_sf(data = coaststates, fill = 'gray80') +
  # 5KM grid with pings. Scale fill color by number of pings
  ...
  theme_minimal()
map_log_pings_dcrb_year_month_5km_df_adjusted

# visualize all years (adjusted ping rate with trip-wise adjustment)
# join summary back to geometry by GRID5KM_ID
dcrb_year_month_5km_df_with_grid <- dcrb_year_month_5km_df %>% 
  left_join(grd, by = join_by(GRID5KM_ID)) %>%
  st_as_sf()
...
map_log_pings_dcrb_year_month_5km_df_adjusted <- ggplot() +
  ...
  # set manual bounding box
  xlim(bbox[1], bbox[3]) + ylim(bbox[2], bbox[4]) +
  labs(fill="log10(Adjusted Tripwise Pings) per 5x5 cell") +
  theme_minimal()
map_log_pings_dcrb_year_month_5km_df_adjusted
...
ping_interval_df %>%
  group_by(year) %>%
  summarise(min(ping_interval_hours, na.rm=TRUE),
            median(ping_interval_hours, na.rm=TRUE),
            mean(ping_interval_hours, na.rm=TRUE),
            max(ping_interval_hours, na.rm=TRUE))

ping_interval_df %>%
  ggplot(aes(x = as.factor(year), y = ping_interval_hours)) + geom_boxplot() + ylim(c(0,2))
```

```{r plot avg. ping rate per ticket}
