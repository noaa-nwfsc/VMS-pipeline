...
Pre-requisite: Run the pipeline steps 1-2 for calendar years 2011-2023 for DCRB.
...
3. Clean up fish ticket data, and double check joined, cleaned, interpolated VMS + fish ticket data is clean

```{r cleaning}

ticket_df_clean <- ticket_df %>% 
  ...
  # remove aquaculture
  #filter(PARTICIPATION_GROUP_CODE != "A") %>% 
  #filter(is.na(FINAL_LENGTH)) %>% # most records do NOT have a vessel length! also true after filtering to DCRB trips only in transform chunk below. so rather tha using this is.na() step, remove duplicated rows for columns not derived from vessel reg database
  distinct(
     Rec_ID, FTID, pacfin_port_code, port_group_code, dealerID, drvid, agency_code, gear_code_all, date, month, year, year_mo, jdate, year_jdate, Week, year_Wk, TARGET_lbs, all_species_lbs, DCRB_lbs, DCRB_lbs_no_revenue, TARGET_rev, all_species_rev, DCRB_revenue, species_code_all,
     .keep_all = TRUE
  )
...
dcrb_ticket_df <- ticket_df_clean %>%
...
*** Note you can skip to the chunk load_dcrb_vms and avoid running chunks 11-16, but you still need the chunk load_10arcminute_grid
...
x <- summarize_dcrb_attribute_block10('trips', 'all years') %>% mutate(metric = 'trips') %>% rename(quant_trips = quant, mean_allyrs_trips = mean_allyrs)
y <- summarize_dcrb_attribute_block10('pings', 'all years') %>% mutate(metric = 'pings') %>% rename(quant_pings = quant, mean_allyrs_pings = mean_allyrs)
...
07-11-2025. now start adding mapping code, see line 808 of Owen's make_map_coastwide()