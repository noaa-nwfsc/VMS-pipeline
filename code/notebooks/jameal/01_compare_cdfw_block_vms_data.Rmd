...
*** Note you can skip to the chunk load_dcrb_tix and avoid running chunks 3-8 

These data can be used to summarize the number/% of tix and number/% of vessels by time period for which CDFW offshore blocks are reported, and examine for structural bias (vessel size, time of year, region, etc)
...
dcrb_ticket_df <- ticket_df2_clean %>%
  ...
  # distinct() %>%
  # add in CDFW block info 'CDFW_AREA_BLOCK' 
  left_join(
    ticket_df2_clean, 
    by = join_by(FTID == FTID, date == date, drvid == VESSEL_NUM, agency_code == AGENCY_CODE) # cannot join on Rec_ID == FISH_TICKET_ID if we wish to match to CDFW_AREA_BLOCK. one challenge is that there are 7 instances (out of 303183 crab trips) of duplicated FTID. JS ignoring for now
    )
...
*** Note you can skip to the chunk load_dcrb_vms and avoid running chunks 11, 13-17, but you still need chunk 12, load_10arcminute_grid
...
# PROBLEM 06-26-2025. with 2011-2023 data, Warning: Detected an unexpected many-to-many relationship between `x` and `y`. but there was no such problem with 2022 data only. argh! 
# to evaluate extent of problem make a df called tmp using lines above except 604-608 (the join to ticket_df2_clean). then dim(tmp) vs dim(dcrb_df)
...
write_rds(dcrb_df, here('Confidential', 
                              'processed_data', 
                              'vms_fish_tickets',
                              paste0(
...
# function to aggregate. 07-10-2025 need to update lines 847-868 to focus on block10 and count up total tix/vms records per block10. make sure not to compare post 2020 high ping rate to pre 2020
