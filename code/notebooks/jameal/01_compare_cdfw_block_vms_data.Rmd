...
ticket_df %>% group_by(year) %>% summarise(n_trips = n_distinct(Rec_ID))
...
dcrb_ticket_df <- ticket_df %>%
  ...
  left_join(
    ticket_df2 %>% filter(VESSEL_NUM != "UNKNOWN") %>% filter(VESSEL_NUM != "") %>% distinct() %>% dplyr::select(FISH_TICKET_ID, LANDING_YEAR, CDFW_AREA_BLOCK), 
    by = join_by(Rec_ID == FISH_TICKET_ID, date == LANDING_YEAR)
    )
