...
test_df <- tibble(year = c(rep(2013, 3), rep(2014, 3)),
                  grid_cell = c(rep(letters[1:3], 2)),
                  n_pings = c(10, 2, 1, 8, 8, 0))
...
This logic is still flawed, I wanted a vectorized way to do this, but can't figure it out for including ties at the threshold cutoff, but not beyond it.

This currently over-includes ties - the last statement in the case when statement will include all ties, not only ties that helped reach the threshold.

I'm going to simplify in the actual persistence map code for now, and forget about including ties for now.
