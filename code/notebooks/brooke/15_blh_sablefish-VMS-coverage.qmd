...
I checked with Kayleigh how she determines if a ticket is a sablefish pot ticket or not, and it's by filtering with the groundfish sector codes and pot group gear code.
...
Weird that the WCGOP/EM data has more rows than the fish ticket data, which should be less complete.
...
This seems off to me, though, because I know there are more rows in the WCGOP/EM data than the fish ticket data, which can't be right. 

Read in raw fish tickets instead, rather than cleaned fish tickets. I wanted to avoid this since it's slow, but I want to use the exact filters Kayleigh suggested rather than the closest available option from VMS output, and see if I get more complete results than with the VMS step 1 cleaned version.
...
write.csv(
  table_raw_df,
  here("Confidential", "sablefish_vms", "table_raw_df.csv")
)
...
I'm worried about the landings and revenue being miscounted or duplicated, since `sabl_raw_tickets` has 1251 rows, but only 681 unique FTID values. I didn't dig into that yet. I'm curious if it's the same issue that led to some duplication in the VMS pipeline, when take was landed for personal vs. commercial use (issue [#19](https://github.com/noaa-nwfsc/VMS-pipeline/issues/19)).