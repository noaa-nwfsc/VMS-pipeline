---
title: "Hindcasts of California Dungeness crab fishing activity, 2015-2023"
...
Pre-requisite: Run the pipeline steps 1-6 (including interpolation) for calendar years 2015-2023.

```{r load-data}
# choose years of data to load
load_years <- 2015:2023

# load VMS and fish ticket data
vms_df <- purrr::map(load_years, function(ly) {
  read_rds(here('Confidential', 'processed', 'matched', 'interpolation',
...
ticket_df <- purrr::map(load_years, function(ly) {
  read_rds(here('Confidential', 'processed', 'fish tickets',
...
time_series_df %>% ggplot() + geom_col(aes(x = year_month, y = n_vms_records, fill = crab_year))
...
time_series_df %>% ggplot() +  geom_col(aes(x = year_month, y = n_unique_vessels, fill = crab_year))
ggsave(filename = here("Confidential", "hindcast_output", "figures", "time_series_n_unique_vessels.png"))

# plot # unique vessels per month as bar chart
time_series_df %>% ggplot() +  geom_col(aes(x = year_month, y = DCRB_lbs, fill = crab_year))
ggsave(filename = here("Confidential", "hindcast_output", "figures", "time_series_DCRB_lbs.png"))

# plot # unique vessels per month as bar chart
time_series_df %>% ggplot() +  geom_col(aes(x = year_month, y = DCRB_revenue_afi, fill = crab_year))
ggsave(filename = here("Confidential", "hindcast_output", "figures", "time_series_DCRB_revenue_afi.png"))
