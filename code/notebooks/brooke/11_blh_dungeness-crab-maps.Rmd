...
# blank map, to fill in gif for months with no data
blank_map <- function(background_sf, map_bbox, title_label, png_name) {
  map_output <- ggplot() +
    geom_sf(data = background_sf, fill = 'gray80') +
...
simple_persistence_map <- fishing_map(
  ...
  title_label = paste0("Persistence Map (Simple), ", min(load_years), "-", max(load_years)),
...
write.csv(file = here("Confidential", "hindcast_output", output_subdir_name, "tables", "dcrb_5km_persistence_ranked_interim_df.csv"), x = dcrb_5km_persistence_ranked_interim_df, row.names = FALSE)
...
# write ranked persistence summary
...
ranked_persistence_map <- fishing_map(
  ...
  title_label = paste0("Persistence Map (", persistence_threshold * 100, "% Effort), ", min(load_years), "-", max(load_years)),
  fill_label = "Crab Years with Fishing Activity",
  png_name = here("Confidential", "hindcast_output",  output_subdir_name, "maps", "persistence", "all", paste0("ranked_persistence_all_time_", persistence_threshold * 100, ".png"))
)
```
...
# write intermediate output
write.csv(file = here("Confidential", "hindcast_output", output_subdir_name, "tables", "dcrb_5km_persistence_ranked_interim_state_df.csv"), x = dcrb_5km_persistence_ranked_interim_state_df, row.names = FALSE)
...
# write ranked persistence summary
write.csv(file = here("Confidential", "hindcast_output", output_subdir_name, "tables", "dcrb_5km_persistence_ranked_state_df.csv"), x = dcrb_5km_persistence_ranked_state_df, row.names = FALSE)
...
for (s in state_vector) {
  ...
  ranked_persistence_map <- fishing_map(
    ...
    title_label = paste0("Persistence Map (", persistence_threshold * 100, "% Effort), ", min(load_years), "-", max(load_years)),
    fill_label = "Crab Years with Fishing Activity",
    png_name = here("Confidential", "hindcast_output",  output_subdir_name, "maps", "persistence", s, paste0("ranked_persistence_all_time_", persistence_threshold * 100, ".png"))
  )
}
```
...
for (r in region_vector) {
  ...
  for (y in year_vector) {
    for (m in 1:12) {
      ...
      # if there is data, map # VMS pings for each month
      if (nrow(ym_data_sf) > 0) {
        vms_map <- fishing_map(
          ...
          fill_label = "Log-10 Interpolated VMS Records",
          png_name = here("Confidential", "hindcast_output",
            output_subdir_name, "maps", "monthly", r, 
            paste0("log_10_n_vms_records_", y, "_", m, ".png")
          )
        )
      # else save a blank map
      } else {
        vms_map <- blank_map(
          background_sf = west_coast_states,
          map_bbox = region_bbox,
          ...
          png_name = here("Confidential", "hindcast_output",
            output_subdir_name, "maps", "monthly", r, 
            paste0("log_10_n_vms_records_", y, "_", m, "_blank.png")
          )
        )
      }
    }
