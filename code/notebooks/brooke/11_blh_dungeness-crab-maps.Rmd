...
# get unique states from gridded fishing summary
region_vector <- sort(unique(dcrb_5km_monthly_sf$map_region))
...
for (r in region_vector) {
  ...
  # get unique years and months from gridded fishing summary
  region_year_month_vector <- sort(unique(region_sf$year_month_character))
  
  # iterate over months
  for (ym in region_year_month_vector) {
    # filter data for given month
    ym_data_sf <- region_sf %>% filter(year_month_character == ym) 
    # make and save map of # VMS pings for each month
    vms_map <- fishing_map(
      data_sf = ym_data_sf,
      fill_var = "log_10_n_vms_records",
      ...
      title_label = paste(substr(ym, 1, 4), month.name[as.numeric(substr(ym, 6, 7))]),
      fill_label = "Log-10 Interpolated VMS Records",
      png_name = here("Confidential", "hindcast_output",  output_subdir_name, "maps", "monthly", r, paste0("log_10_n_vms_records_", ym, ".png"))
    )
  }
}
```
