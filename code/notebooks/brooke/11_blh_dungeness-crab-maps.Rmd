...
pcepi_adjustment <- read.csv(file = here("Confidential", "hindcast_output", output_subdir_name, "tables", "inflation_adjustment_pcepi.csv"))
```
...
dcrb_vms_df <- vms_df %>%
  ...
  left_join(pcepi_adjustment, by = join_by(year_numeric == year)) %>%
...
dcrb_5km_yearly_df <- dcrb_5km_monthly_df %>%
  group_by(GRID5KM_ID, crab_year_character) %>%
...
ranked_persistence_map <- fishing_map(
  ...
  title_label = paste0("Ranked Persistence Map, ", min(load_years), "-", max(load_years)),
  fill_label = "Crab Years with Fishing Activity",
