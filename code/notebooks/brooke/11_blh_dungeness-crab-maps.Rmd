...
# calculate pings per grid cell per crab year
dcrb_5km_yearly_df <- dcrb_5km_monthly_df %>%
  group_by(GRID5KM_ID, crab_year_character) %>%
  summarise(n_vms_records = sum(n_vms_records))

# rank grid cells based on # pings per crab year and decide whether to include in persistence map
dcrb_5km_persistence_ranked_interim_df <- dcrb_5km_yearly_df %>% 
  ...
  mutate(cumulative_n_vms_records = cumsum(n_vms_records),
         include_1 = (cumulative_n_vms_records <= year_cutoff_n_vms_records),
         include_2 = ifelse(!include_1 & 
                              lag(include_1) &
                              (lag(cumulative_n_vms_records) < year_cutoff_n_vms_records) & 
                              cumulative_n_vms_records > year_cutoff_n_vms_records, 
                            TRUE,
                            include_1),
         include_3 = ifelse(!include_2 & 
                              lag(include_2) &
                              lag(n_vms_records) == n_vms_records,
                            TRUE,
                            include_2))
