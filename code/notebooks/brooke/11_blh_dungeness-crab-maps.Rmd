...
write.csv(file = here("Confidential", "hindcast_output", output_subdir_name, "tables", "dcrb_5km_monthly_df.csv"), x = dcrb_5km_monthly_df, row.names = FALSE)
```
...
# write simple persistence summary
write.csv(file = here("Confidential", "hindcast_output", output_subdir_name, "tables", "dcrb_5km_persistence_simple_df.csv"), x = dcrb_5km_persistence_simple_df, row.names = FALSE)
...
simple_persistence_map <- fishing_map(
  ...
  title_label = paste0("Simple Persistence Map, ", min(load_years), "-", max(load_years)),
  fill_label = "Crab Years with Fishing Activity",
  png_name = here("Confidential", "hindcast_output",  output_subdir_name, "maps/persistence/all", paste0("simple_persistence_all_time.png"))
)

# TODO monthly across years, annually across months
# TODO statewise
```

Create a ranked persistence map, which will visualize the top x% of fishing activity based on the number of interpolated pings for a given year in a given grid cell. There is some complicated logic in here, see `12_blh_example-ranked-persistence-map.Rmd` for a hypothetical example.
...
# TODO make sure this works with a clean starting environment (restart R and run up to this cell)

# set a threshold of fishing activity for a given year you'd like to reach
persistence_threshold <- 0.75

# calculate pings per crab year
...
# rank pings in a given year and decide whether to include in map for that year based on contribution to annual fishing activity
dcrb_5km_persistence_ranked_df <- dcrb_5km_monthly_df %>% 
  left_join(dcrb_5km_yearly_df, by = 'crab_year_character') %>% 
  group_by(crab_year_character) %>%
  # rank each grid cell based on # interpolated VMS pings for that crab year
  mutate(dense_rank_n_vms_records = dense_rank(n_vms_records * -1)) %>% 
  # sort that crab year's data based on the rank
  arrange(crab_year_character, dense_rank_n_vms_records) %>%
  # calculate the cumulative sum of records to see which grid cells count towards the persistence threshold that year
  mutate(cumulative_n_vms_records = cumsum(n_vms_records),
         include_in_map = case_when(
           # if we haven't reached the threshold yet, then include the grid cell
           cumulative_n_vms_records <= year_cutoff_n_vms_records ~ TRUE,
           # if we have reached the threshold but it happened in between this grid cell and the prior grid cell, then include the grid cell
           lag(cumulative_n_vms_records) < year_cutoff_n_vms_records ~ TRUE,
           # if we have reached the threshold and it didn't happen with this grid cell, but this grid cell is tied with the grid cell that put it over, then include this grid cell
           n_vms_records == lag(n_vms_records) ~ TRUE,
           .default = FALSE
         )) %>%
  # if some grid cells were included after reaching the threshold just because they were ties, then exclude those grid cells
  mutate(include_in_map_corrected = coalesce(ifelse(!lag(include_in_map), FALSE, include_in_map), include_in_map)) %>% 
  # remove the uncorrected version of include_in_map so there's less confusion with the output
  select(-include_in_map)

# write intermediate output
write.csv(file = here("Confidential", "hindcast_output", output_subdir_name, "tables", "dcrb_5km_persistence_ranked_df.csv"), x = dcrb_5km_persistence_ranked_df, row.names = FALSE)
