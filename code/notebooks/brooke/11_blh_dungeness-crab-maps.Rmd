...
Assign coordinate reference systems for the different maps. The values include 'Coast' and the regions that are options for region_vector at the top of the script.
...
fishing_map <- function(data_sf, fill_var, fill_range, fathoms_sf, background_sf, map_bbox, title_label, fill_label, png_name) {
  # make plot
  map_output <- ggplot() +
...
coast_bbox <- st_transform(x = dcrb_5km_monthly_sf, crs = crs_list[['Coast']]) %>%
  st_bbox()
...
for (ym in year_month_vector) {
  ...
  ym_data_sf <- dcrb_5km_monthly_sf %>% 
    filter(year_month_character == ym) %>%
    st_transform(crs = crs_list[['Coast']])
  ...
  for (y in year_vector) {
    for (m in 1:12) {
      ...
      vms_map <- fishing_map(
        ...
        title_label = paste(r, y, month.name[m]),
        fill_label = "Log-10 Interpolated VMS Records",
        png_name = png_name
      )
    }
