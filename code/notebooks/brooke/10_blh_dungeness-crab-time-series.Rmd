...
Adjust revenue for inflation using FRED Personal Consumption Expenditures: Chain-Type Price Index, [PCEPI](https://fred.stlouisfed.org/series/PCEPI). Code is adapted from R code by Erin Steiner. PCEPI index is used per Dan Holland's suggestion. This product uses the FRED® API but is not endorsed or certified by the Federal Reserve Bank of St. Louis. The reference year is 2017 by default.

Pre-requisite: Create a FRED account and [API key](https://fred.stlouisfed.org/docs/api/api_key.html).
...
# generate the mean annual deflators based on the monthly values
pcepi_adjustment <- mutate(fred_pcepi, year = year(date)) %>%
  group_by(year) %>%
  summarize(pcepi = mean(value), .groups = 'drop') %>%
  mutate(inflation_adjustment_factor = pcepi / pcepi[year == max(year)]) %>%
  select(year, inflation_adjustment_factor)

# write inflation adjustment factors
write.csv(file = here("Confidential", "hindcast_output", output_subdir_name, "tables", "inflation_adjustment_pcepi.csv"), x = pcepi_adjustment, row.names = FALSE)

# plot inflation adjustment factors
pcepi_adjustment %>% ggplot(aes(x = as.factor(year), y = inflation_adjustment_factor)) + geom_point()
ggsave(here("Confidential", "hindcast_output", output_subdir_name, "figures", "inflation_adjustment_pcepi.png"))

# clean up
rm(fred_pcepi)
```

The reference year for inflation adjustment is `r max(pcepi_adjustment$year)`.
...
ggsave(filename = here("Confidential", "hindcast_output", output_subdir_name, "figures", paste0("time_heatmap_n_unique_vessels_coast.png")))
