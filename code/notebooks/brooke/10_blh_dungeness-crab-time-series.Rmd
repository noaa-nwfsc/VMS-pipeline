...
# write time series
write.csv(file = here("Confidential", "hindcast_output", output_subdir_name, "tables", "time-series.csv"), x = time_series_df, row.names = FALSE)
```
...
# cumulative time series line plot
time_series_df %>% 
  pivot_longer(cols = c('cumulative_sum_DCRB_lbs', 'cumulative_sum_DCRB_revenue', 'cumulative_sum_DCRB_revenue_afi'), names_to = "variable") %>% 
  ggplot() + 
  geom_line(aes(x = year_month_date, y = value, color = crab_year_character)) +
  scale_x_date(breaks = df_date_breaks, date_labels = "%b %Y") +
  facet_grid(variable ~ ., scales = "free_y")
ggsave(filename = here("Confidential", "hindcast_output", output_subdir_name, "figures", paste0("time_series_line_cumulative.png")))
```

```{r}
# non-cumulative time series line plot
time_series_df %>% 
  pivot_longer(cols = c('n_fish_tickets', 'n_unique_vessels', 'DCRB_lbs', 'DCRB_revenue', 'DCRB_revenue_afi'), names_to = "variable") %>% 
  ggplot() + 
  geom_line(aes(x = year_month_date, y = value, color = crab_year_character)) +
  scale_x_date(breaks = df_date_breaks, date_labels = "%b %Y") +
  facet_grid(variable ~ ., scales = "free_y")
ggsave(filename = here("Confidential", "hindcast_output", output_subdir_name, "figures", paste0("time_series_line_non_cumulative.png")))
```
...
time_series_df %>% 
  ggplot() + 
  geom_tile(aes(x = month_factor, y = as.factor(year_numeric), fill = n_unique_vessels))
ggsave(filename = here("Confidential", "hindcast_output", output_subdir_name, "figures", paste0("time_series_heatmap_n_unique_vessels.png")))
```
