...
keepers <- c('process_year','first_process_year','alltime','spp_codes','gear_codes','pacfin_weight_metric','pacfin_revenue_metric','target_cutoff','lookback_window','year_begin','year_end','lookback_begin', 'interpolate_flag', 'output_dir_name')
...
fishtix <- read_rds(here::here('Confidential', 'data', output_dir_name, 'fish_tickets', paste0('fishtix_vlengths_withFTID_', process_year, '.rds')))
...
if(process_year > first_process_year){
  fishtix_prev <- read_rds(here::here('Confidential', 'data', output_dir_name, 'fish_tickets', paste0('fishtix_withFTID_', process_year-1, '.rds'))) %>% 
...
```{r load vms}
vms <- read_rds(here::here('Confidential', 'data', output_dir_name, 'vms', paste0('vms_clean_', process_year, '.rds')))

if(process_year > first_process_year){
  vms_prev <- read_rds(here::here('Confidential', 'data', output_dir_name, 'vms', paste0('vms_clean_', process_year-1, '.rds'))) %>% 
...
output_subdir <- here('Confidential','data', output_dir_name, 'matched')
if (!dir.exists(output_subdir)) dir.create(output_subdir)

# write both versions
write_rds(fishtix_vms_all, here::here('Confidential', 'data', output_dir_name, 'matched', paste0('matched_alltix_withFTID_', process_year, '.rds')))
write_rds(fishtix_vms_only, here::here('Confidential', 'data', output_dir_name, 'matched', paste0('matched_vmstix_only_withFTID_', process_year, '.rds')))
```
