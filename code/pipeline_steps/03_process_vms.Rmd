...
vms_fn <- list.files(here('Confidential','data','raw','vms'),full.names = T)
...
output_subdir <- here('Confidential','data', output_dir_name, 'vms')
if (!dir.exists(output_subdir)) dir.create(output_subdir)

# write output
write_rds(vms_dupes, here::here('Confidential', 'data', output_dir_name, 'vms', paste0('vms_duplicates_', process_year, '.rds')))
```
...
```{r save rds}
write_rds(vms_ordered,here::here('Confidential','data', output_dir_name, 'vms', paste0('vms_clean_', process_year, '.rds')))
```
