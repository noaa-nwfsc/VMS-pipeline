...
log_lines <- read_lines(
  here('code', 'pipeline_steps', 'log', most_recent_log_file),
  skip = 15,
  n_max = 2
)

# find output directory name and process year from most recent log file
log_line_output_dir_name <- log_lines[which(startsWith(
  log_lines,
  "output_dir_name"
))]
...
file.rename(
  from = here('code', 'pipeline_steps', 'main_process.html'),
  to = here(
    'Confidential',
    'processed_data',
    output_dir_name,
    'markdowns',
    paste0('main_process_', process_year, '.html')
  )
)