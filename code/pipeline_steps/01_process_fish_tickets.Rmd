...
rawdat <- read_rds(here::here('Confidential','data','raw','fish_tickets','all_fishtickets_1994_2023.rds'))
rawdat <- rawdat %>%
...
# create output directory, if doesn't yet exist
output_dir <- here('Confidential', 'data', output_dir_name)
...
for(y in 1:length(yrs_of_data)) {
  ...
  output_subdir <- here('Confidential', 'data', output_dir_name, 'fish_tickets')
  if (!dir.exists(output_subdir)) dir.create(output_subdir)
  
  # write output
  dat_out %>% 
    filter(year==yrs_of_data[y]) %>% 
    write_rds(here::here('Confidential','data', output_dir_name, 'fish_tickets', paste0("fishtix_withFTID_", yrs_of_data[y], ".rds", collapse=""))) 
}
