...
landings <- purrr::map(process_year, function(y){
  read_rds(here::here('Confidential','data', output_dir_name, 'fish_tickets', paste0('fishtix_withFTID_', y, '.rds')))
}) %>% 
  bind_rows()
...
for(y in unique(vessel_length_key_df$year)){
  ...
  key_subset <- vessel_length_key_df %>%
    filter(year == y)
  write_rds(key_subset, here::here('Confidential', 'data', output_dir_name, 'vessel_length_keys', paste0('vessel_length_key_', y, '.rds')))
}
```
...
for(y in unique(vessel_length_key_df$year)){
  ...
  write_rds(landings_length,here::here('Confidential', 'data', output_dir_name, 'fish_tickets', paste0('fishtix_vlengths_withFTID_', y, '.rds')))
}
```
