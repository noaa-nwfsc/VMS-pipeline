...
```{r source_functions}
source(here::here('code','piipeline_steps',"Report_Vessel_Length_processedtix_functions.R"))
```
...
landings <- purrr::map(process_year, function(y){
  read_rds(here::here('Confidential','data','processed','fish_tickets',paste0(y,'fishtix_withFTID.rds')))
}) %>% 
  bind_rows()
...
for(y in unique(vessel_length_key_df$year)){
  key_subset <- vessel_length_key_df %>%
    filter(year == y)
  write_rds(key_subset,here::here('Confidential','data','processed','vessel length keys',paste0('vessel_length_key_', y, '.rds')))
}
```
...
for(y in unique(vessel_length_key_df$year)){
  ...
  write_rds(landings_length,here::here('Confidential','data','processed','fish_tickets',paste0(y, 'fishtix_vlengths_withFTID.rds')))
}
```
