...
For species code options, see [PacFIN Species Code List](https://reports.psmfc.org/pacfin/f?p=501:811:12652162289843:INITIAL::). 

For gear code options, see [PacFIN Gear Code List](https://reports.psmfc.org/pacfin/f?p=501:804:15350132650778:INITIAL::). Use the full names rather than abbreviations (i.e., the "Gear Description" column of the linked table, e.g. "CRAB POT", not "CPT").

```{r}
# species codes

# spp_codes <- c("SPRW") # CA spot prawn
# spp_codes <- c("DCRB") # Dungeness crab
# spp_codes <- c("CHNK") # Chinook salmon
# spp_codes <- c("ALBC") # Albacore tuna
# spp_codes <- c("DOVR") # Dover sole (if running just for LE groundfish bottom trawl)
# spp_codes <- c("PSHP") # pink shrimp
spp_codes <- c("SABL") # sablefish

# spp_codes_name <- "DCRB_CHNK"

# gear codes

# gear_codes <- c("PRAWN TRAP","CRAB OR LOBSTER POT") # CA spot prawn
# gear_codes <- c("SHELLFISH POT (CRAB)","CRAB OR LOBSTER POT","CRAB POT","CRAB RING") # Dungeness crab
# gear_codes <- c("TROLL, (SALMON)","OCEAN TROLL","TROLL (SALMON)","DRAG SEINE","DIP BAG NET","HOOK AND LINE","PURSE SEINE (SALMON)") # Chinook salmon
# gear_codes <- c("OCEAN TROLL","TROLL (SALMON)","TROLL (ALBACORE)","HOOK AND LINE","JIG (ALBACORE)") # Albacore tuna
# gear_codes <- c("BALLOON TRAWL","BEAM TRAWL","bottom trawl - mud(sole) gear","bottom trawl - roller gear","DANISH/SCOTTISH SEINE (TRAWL)","GROUNDFISH TRAWL","GROUNDFISH TRAWL, SMALL FOOTROPE (< 8 in.)","GROUNDFISH(OTTER) TRAWL","OTTER TRAWL","PAIR TRAWL","PARANZELLA","ROLLER TRAWL","SELECTIVE FLATFISH TRAWL","SELECTIVE FLATFISH TRAWL, SMALL FOOTROPE","TRAWL NET","TRAWL, FOOTROPE GREATER THAN 8 INCHES IN DIAMETER","TRAWL, FOOTROPE LESS THAN 8 INCHES IN DIAMETER") # groundfish bottom trawl only
# gear_codes <- c("SINGLE RIGGED SHRIMP TRAWL","DOUBLE RIGGED SHRIMP TRAWL","SHRIMP TRAWL (SR)","SHRIMP TRAWL (DR)","GULF SHRIMP TRAWL","BEAM TRAWL") # pink shrimp
gear_codes <- c("FISH POT", "OTHER POT GEAR", "ALL POT AND TRAP GEAR") # sablefish pot
```
...
```{r step4, message=F, warning=F, child=here::here('code', 'pipeline_steps', '04_match_vms_fishtix.Rmd')}
```
```{r}
x<-proc.time()-alltime
