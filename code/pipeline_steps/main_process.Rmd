...
```{r step1,message=F,warning=F,child=here::here('code', 'process steps','01_process_fish_tickets.Rmd')}
```
```{r}
x<-proc.time()-alltime
```
...
```{r step2, message=F,warning=F,child=here::here('code', 'process steps','02_calculate_vessel_lengths.Rmd')}
```
```{r}
x<-proc.time()-alltime
```
...
```{r step3, message=F,warning=F,child=here::here('code', 'process steps','03_process_vms.Rmd')}
```
```{r}
x<-proc.time()-alltime
```
...
```{r step5, message=F,warning=F,child=here::here('code', 'process steps','04_match_vms_fishtix.Rmd')}
```
```{r}
x<-proc.time()-alltime
...
```{r step5, message=F,warning=F,child=here::here('code', 'process steps','05_filter_matched_data.Rmd')}
```
```{r}
x<-proc.time()-alltime
```
...
```{r step6, message=F,warning=F,child=if (interpolate_flag) here::here('code', 'process steps','optional_06_interpolate.Rmd')}
```
```{r}
x<-proc.time()-alltime
```
